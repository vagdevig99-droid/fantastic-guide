...
function renderClassic() {
  ...
  c.quickActions.forEach((qa) => {
    ...
    node.innerHTML = `<span class="qa-icon">${qa.icon}</span><span class="qa-label">${qa.label}</span>`;
...
function f_row(b) {
  ...
  n.innerHTML = `
    <div class="f-row-icon">${b.icon || "•"}</div>
...
async function probeLiveBackend() {
  ...
  try {
    ...
    if (data.ok && data.live_ready) {
      ...
      state.mode = "live";
      // No visible UI change — the live path activates silently for any
      // subsequent chat message or hero-chip start.
    }
  } catch (e) {
...
async function runLiveTurn(userText) {
  ...
  try {
    ...
    while (true) {
      ...
      while ((blankIdx = buffer.indexOf("\n\n")) >= 0) {
        ...
        if (evt === "reasoning") {
          ...
          ensureBubble().insertAdjacentHTML(
            "beforeend",
            `<p style="color:#FB7299"><b>Error:</b> ${data.message || "unknown"}</p>`
          );
        } else if (evt === "done") {
          // close out below
        }
    ...
    finishReasoning();
    appendAriaPlain(
      `<p>I couldn't reach the live agent (${e.message}). Switch back to <b>Scripted</b>, or check that the server is running (<code>python server.py</code>).</p>`
    );
  }
